...
def demo(data=os.getenv('DATA_DIR'), save='/tmp', depth=40, growth_rate=12, efficient=True,
    ...
    if data == 'cifar10':
        ...
        valid_set = tv.datasets.CIFAR10(data_root, train=True, transform=test_transforms, download=True)
        test_set = tv.datasets.CIFAR10(data_root, train=False, transform=test_transforms, download=True)
    if data == 'svhn':
        data_root = data
